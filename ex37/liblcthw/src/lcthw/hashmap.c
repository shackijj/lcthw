...
{
    return bstrcmp((bstring)a, (bsring)b);
}
...
{
    Hashmap *map = calloc(1, sizof(Hashmap));
    chech_mem(map);
         ...
         int create, uint32_t *hash_out)
{
    ...
    chech(bucket_n >= 0, "Invalid bucket found: %d", bucket_n);
...
int Hashmap_traverse(Hashmap *map, Hashmap_traverse_cb traverse cb)
{
    int i = 0;
    int j = 0;
    int rc = 0;
...
void *Hashmap_delete(Hashmap *map, void *key)
{
    ...
    int i = Hash_map_get_node(map, hash, bucket, key);
    if(i == -1) return NULL;
